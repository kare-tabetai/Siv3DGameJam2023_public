...
{
	float3 g_fogColor;
	double g_fogCoefficient;
}
// [C++]
//struct PSFog
//{
//	Float3 fogColor;
//	double fogCoefficient;
//};
...
float3 CalculateSpecularReflection(float3 n, float3 h, double shininess, double nl, float3 lightColor, float3 specularColor)
{
	const double highlight = pow(saturate(dot(n, h)), shininess) * double(0.0 < nl);
...
float4 PS(s3d::PSInput input) : SV_TARGET
{
	...
	const double fogFactor = exp(-g_fogCoefficient * distance(g_eyePosition, input.worldPosition));
