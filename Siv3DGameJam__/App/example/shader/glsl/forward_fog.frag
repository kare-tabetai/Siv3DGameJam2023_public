...
vec3 CalculateSpecularReflection(vec3 n, vec3 h, double shininess, double nl, vec3 lightColor, vec3 specularColor)
{
	double highlight = pow(max(dot(n, h), 0.0f), shininess) * double(0.0f < nl);
...
void main()
{
	...
	double fogFactor = exp(-g_fogCoefficient * distance(g_eyePosition, WorldPosition));
